...
if __name__ == '__main__':
    ...
    try:
        ...
        n_users = n_params['n_users']  # user
        n_items = n_params['n_items']  # item
        n_entities = n_params['n_entities']  # entity
        n_relations = n_params['n_relations']  # i ---relation---> e
        n_nodes = n_params['n_nodes']  # nodes
        ...
        for epoch in range(args.epoch):
            """training CF"""
            """cf data"""
            # train_cf = defaultdict(list) user -> list[item]
            train_cf_with_neg = neg_sampling_cpp(train_cf, user_dict['train_user_set'])  ## 隨機採樣負樣本
            ...
            for iter in tqdm(range(1, n_kg_batch + 1)):
                kg_batch_head, kg_batch_relation, kg_batch_pos_tail, kg_batch_neg_tail = generate_kg_batch(kg_dict,
                                                                                                           4096,
                                                                                                           n_params[
                                                                                                               'n_entities'])
                ...
                kg_batch_loss = model.calc_kg_loss_transE(kg_batch_head, kg_batch_relation, kg_batch_pos_tail,
                                                          kg_batch_neg_tail)
            ...
            if epoch % test_interval == 0 and epoch >= 0:
                ...
                logger.info('{}: using time {:.1f}, training loss at epoch {}: {}'.format(
